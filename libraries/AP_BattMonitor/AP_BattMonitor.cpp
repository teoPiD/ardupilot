...
AP_BattMonitor::init()
{
    ...
    for (uint8_t instance=0; instance<AP_BATT_MONITOR_MAX_INSTANCES; instance++) {
        ...
        switch (get_type(instance)) {
            ...
            case AP_BattMonitor_Params::BattMonitor_TYPE_CUSTOM:
                _params[instance]._i2c_bus.set_default(AP_BATTMONITOR_SMBUS_BUS_EXTERNAL);
                drivers[instance] = new AP_BattMonitor_SMBus_Custom(*this, state[instance], _params[instance],
                                                                    hal.i2c_mgr->get_device(_params[instance]._i2c_bus, _params[instance]._i2c_address,
                                                                                            100000, true, 20), _params[instance]._num_cells);
                break;
